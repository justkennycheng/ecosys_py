...
class Organism:
    ...
    def __init__(self, o_id, settings):
        ...
        self.max_hunger = ...     #当前年龄的最大饱腹度,Calculate_01()中依据当前年龄计算得出。 兔子幼年最大饱腹会降低，因而需要更频繁的进食。类似速度的算法。
        self.max_energy = ...     #当前年龄的最大能量,Calculate_01()中依据当前年龄计算得出。 兔子幼年的最大能量会降低，因而需要更频繁的休息。
        self.hunger_TH = ...      #饥饿阈值  hungerThreshold = hungerDesireThreshold_factor * currentHungerUpperlimit
        self.energy_TH = ...      #能量阈值   energyThreshold = energyDesireThreshold_factor * currentEnergyUpperlimit
